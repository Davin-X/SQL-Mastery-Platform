...
INSERT INTO employee_dept VALUES (1, 'Davin', 'ENG', 15000);
    ...
    CASE
        ...
        ELSE (
            ...
            WHERE
                ...
                AND e2.salary < (
                    SELECT MAX(salary)
                    FROM employee_dept e3
                    WHERE
                        e2.department = e3.department
                )
        )
...
SELECT DISTINCT
    department,
    CASE
        WHEN COUNT(*) >= 2 THEN MAX(salary) OVER (
            PARTITION BY
                department
            ORDER BY
